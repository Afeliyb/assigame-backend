...
public class MessageService {
    ...
    public MessageDTO envoyerMessage(
        ...
        if (contenu.length() > 2000) {
            throw new RuntimeException("Le message ne peut pas dépasser 2000 caractères.");
        }
    ...
    public List<ConversationSummaryDTO> getBoiteDeReception(Long idUtilisateur) {
        // Remplacement de la requête native problématique par un Set Java
        Set<Long> interlocuteurIds = new HashSet<>();
        interlocuteurIds.addAll(messageRepository.findDestinatairesIds(idUtilisateur));
        interlocuteurIds.addAll(messageRepository.findExpediteursIds(idUtilisateur));
        ...
        for (Long interlocuteurId : interlocuteurIds) {
            ...
            if (interlocuteurOpt.isEmpty()) continue;

            Utilisateur interlocuteur = interlocuteurOpt.get();

            List<Message> derniers = messageRepository.findLastMessage(idUtilisateur, interlocuteurId);
            if (derniers.isEmpty()) continue;
